...
function lean_content_nav( $nav_id ) {
	...
	<nav role="navigation" id="<?php echo $nav_id; ?>" class="<?php echo $nav_class; ?>">
		<h1 class="assistive-text"><?php _e( 'Post navigation', 'lean' ); ?></h1>
...
function lean_comment( $comment, $args, $depth ) {
	...
	<li class="post pingback">
		<p><?php _e( 'Pingback:', 'lean' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( '(Edit)', 'lean' ), ' ' ); ?></p>
	<?php
			break;
		default :
	?>
		...
		<article id="comment-<?php comment_ID(); ?>" class="comment">
			<footer>
				<div class="comment-author vcard">
					...
					<?php printf( __( '%s <span class="says">says:</span>', 'lean' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
				</div><!-- .comment-author .vcard -->
				<?php if ( $comment->comment_approved == '0' ) : ?>
					<em><?php _e( 'Your comment is awaiting moderation.', 'lean' ); ?></em>
					<br />
				<?php endif; ?>
				...
				<div class="comment-meta commentmetadata">
					...
					<?php
						...
						printf( __( '%1$s at %2$s', 'lean' ), get_comment_date(), get_comment_time() ); ?>
					</time></a>
...
endif; // ends check for lean_comment()
...
function lean_posted_on() {
	printf( __( '<time class="entry-date" datetime="%3$s" pubdate>%4$s</time>', 'lean' ),
		...
		esc_attr( sprintf( __( 'View all posts by %s', 'lean' ), get_the_author() ) ),
